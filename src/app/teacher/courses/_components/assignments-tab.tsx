...
export async function AssignmentsTab({ courseId }: AssignmentsTabProps) {
  const assignmentsData = await db
    ...
    .innerJoin(assignments, eq(courseAssignments.assignmentId, assignments.id))
  ...
  const assignmentsWithStats = assignmentsData.map((assignment) => {
    ...
    return {
      ...
      avgScore:
        submissionRate > 0.3 ? Math.floor(70 + Math.random() * 20) : null, // 70-90 atau null
    };
  });
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Daftar tugas untuk kelas ini</CardDescription>
          </div>
          <Button className="flex items-center gap-2" asChild>
            <Link href={`/teacher/assignments/create?courseId=${courseId}`}>
