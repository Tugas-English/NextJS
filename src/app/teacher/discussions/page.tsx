    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground text-xs">+3 bulan ini</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground text-xs">+45 bulan ini</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
      ...
      <Tabs defaultValue="all-discussions">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="all-discussions">Semua Diskusi</TabsTrigger>
          <TabsTrigger value="recent-posts">Postingan Terbaru</TabsTrigger>
          <TabsTrigger value="my-discussions">Diskusi Saya</TabsTrigger>
        </TabsList>
        ...
        <div className="mt-6 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <select className="border-input bg-background h-9 rounded-md border px-3 text-sm">
              ...
              <option value="most-participants">Partisipan Terbanyak</option>
            </select>
          </div>
                  ...
                  {discussionThreads.map((thread) => (
                    <TableRow key={thread.id}>
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium">
                              {thread.title}
                              {thread.isPinned && (
                                <Pin className="text-muted-foreground ml-1 inline-block h-3 w-3" />
                              )}
                              {thread.isLocked && (
                                <Lock className="text-muted-foreground ml-1 inline-block h-3 w-3" />
                              )}
                            </span>
                          </div>
                      ...
                      <TableCell>
                        ...
                        {thread.category === 'module' && <Badge>Modul</Badge>}
                        {thread.category === 'challenge' && (
                          <Badge variant="destructive">Tantangan</Badge>
                        )}
                      </TableCell>
                      <TableCell>{thread.postsCount}</TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem>Lihat Diskusi</DropdownMenuItem>
                            <DropdownMenuItem>Edit</DropdownMenuItem>
                            {thread.isPinned ? (
                              <DropdownMenuItem>Lepas Pin</DropdownMenuItem>
                            ) : (
                              <DropdownMenuItem>Pin Diskusi</DropdownMenuItem>
                            )}
                            {thread.isLocked ? (
                              <DropdownMenuItem>Buka Kunci</DropdownMenuItem>
                            ) : (
                              <DropdownMenuItem>Kunci Diskusi</DropdownMenuItem>
                            )}
                ...
                {recentPosts.map((post) => (
                  <div key={post.id} className="rounded-md border p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="outline">{post.likesCount} suka</Badge>
                    </div>
                    <div className="mt-3">
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {discussionThreads
                    .filter((thread) => thread.author.name === 'Ms. Johnson')
                    .map((thread) => (
                      <TableRow key={thread.id}>
                        <TableCell>
                          <div className="space-y-1">
