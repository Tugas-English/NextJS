...
export default function RubricDetailPage() {
  ...
  const [editCriterionDialogOpen, setEditCriterionDialogOpen] = useState(false);
  ...
  const handleSaveCriterion = () => {
    ...
    setRubric({
      ...
      criteria: rubric.criteria.map((c) =>
        c.id === currentEditingCriterion.id ? currentEditingCriterion : c,
      ),
      updatedAt: new Date().toISOString(),
    });
    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-2">
          ...
          {editMode ? (
            <div className="flex gap-2">
              ...
              <Button onClick={handleSaveRubric}>Simpan Perubahan</Button>
            </div>
          ) : null}
        </div>
      </div>
        ...
        <CardHeader>
          <div className="flex justify-between">
            <div>
              ...
              <CardDescription>
                Dibuat oleh {rubric.createdBy} pada {rubric.createdAt}
                {rubric.updatedAt !== rubric.createdAt && (
                  <span> • Diperbarui pada {rubric.updatedAt}</span>
                )}
              </CardDescription>
            </div>
            <div className="text-right">
              <div className="text-muted-foreground text-sm">Skor Maksimum</div>
              {editMode ? (
                <Input
                  type="number"
                  min={1}
                ...
                <div className="text-xl font-bold">{rubric.maxScore}</div>
              )}
            </div>
          </div>
        </CardHeader>
      ...
      <Tabs defaultValue="criteria">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="criteria">Kriteria Penilaian</TabsTrigger>
            ...
            <Card key={criterion.id}>
              <CardHeader className="pb-2">
                <div className="flex justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="outline">Bobot: {criterion.weight}%</Badge>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
              ...
              <CardContent>
                <div className="grid grid-cols-4 gap-2">
                  {criterion.levels.map((level) => (
                    <div key={level.value} className="rounded-md border p-3">
                      <div className="mb-1 flex items-center justify-between">
                        <div className="font-medium">Level {level.value}</div>
                        <Badge variant="secondary">{level.score} poin</Badge>
                      </div>
        ...
        <TabsContent value="usage" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Rubrik ini digunakan pada tugas dan modul berikut
              </CardDescription>
            </CardHeader>
            <CardContent>
              {rubric.usedIn.length === 0 ? (
                <p className="text-muted-foreground py-8 text-center">
                  Rubrik ini belum digunakan pada tugas atau modul manapun.
                </p>
              ) : (
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span>{currentEditingCriterion.weight}%</span>
                </div>
                <Slider
                ...
                <p className="text-muted-foreground text-xs">
                  Bobot menentukan kontribusi kriteria ini terhadap nilai total
                </p>
              </div>

              <div>
                <label className="text-sm font-medium">Level Penilaian</label>
                <div className="mt-2 space-y-3">
                  {currentEditingCriterion.levels.map((level, index) => (
                    <div
                      key={index}
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {rubric.name}&quot;? Tindakan ini tidak dapat dibatalkan.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            <Button variant="destructive" onClick={confirmDeleteRubric}>
              Hapus
            </Button>
          </DialogFooter>
        </DialogContent>
