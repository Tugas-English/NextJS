...
export function ActivitiesTable({ promises }: ActivitiesTableProps) {
      ...
      try {
        ...
        switch (action.variant) {
          case 'view':
            ...
            break;
          case 'update':
            router.push(`/teacher/activities/${action.row.id}/edit`);
            break;
          case 'delete':
            setRowAction(action);
            break;
  ...
  React.useEffect(() => {
    try {
      ...
      console.error('Gagal melakukan navigasi ke halaman aktivitas:', error);
    }
  }, [router, rowAction]);
  ...
  return (
    <>
      ...
      <DeleteActivitiesDialog
        ...
        activities={rowAction?.row.original ? [rowAction?.row.original] : []}
        showTrigger={false}
