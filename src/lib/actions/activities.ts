...
export async function updateActivity(
  ...
  try {
    ...
    return {
      activity: null,
      error:
        error instanceof Error ? error.message : 'Gagal memperbarui aktivitas',
    };
  }
}
...
export async function getActivityById(id: string) {
  const cachedGetActivity = cache(
    ...
    {
      tags: [CACHE_TAGS.ACTIVITY_DETAIL, `${CACHE_TAGS.ACTIVITY_DETAIL}-${id}`],
      ...
      try {
        ...
        if (hotsType) {
          if (Array.isArray(hotsType)) {
            whereConditions.push(inArray(activities.hotsType, hotsType));
          } else {
        ...
        if (difficulty) {
          if (Array.isArray(difficulty)) {
            whereConditions.push(inArray(activities.difficulty, difficulty));
          } else {
            whereConditions.push(eq(activities.difficulty, difficulty));
          }
        }
        ...
        if (isPublished !== undefined) {
          whereConditions.push(eq(activities.isPublished, isPublished));
        }
        ...
        if (createdAtRange && createdAtRange.length > 0) {
          if (createdAtRange.length === 1) {
            ...
            whereConditions.push(gte(activities.createdAt, startOfDay));
            whereConditions.push(lte(activities.createdAt, endOfDay));
            ...
            whereConditions.push(gte(activities.createdAt, startDate));
            whereConditions.push(lte(activities.createdAt, endDate));
          }
        }
        ...
        if (updatedAtRange && updatedAtRange.length > 0) {
          if (updatedAtRange.length === 1) {
            ...
            whereConditions.push(gte(activities.updatedAt, startOfDay));
            whereConditions.push(lte(activities.updatedAt, endOfDay));
            ...
            whereConditions.push(gte(activities.updatedAt, startDate));
            whereConditions.push(lte(activities.updatedAt, endDate));
          }
        }
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      if (activitiesToDelete.length === 0) {
        throw new Error('Tidak ada aktivitas yang ditemukan untuk dihapus');
      }
  ...
  try {
    ...
    if (userId && activity.createdBy !== userId) {
      return {
        activity: null,
        error:
          'Anda tidak memiliki izin untuk mengubah status publikasi aktivitas ini',
      };
    }
