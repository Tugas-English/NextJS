...
const getCacheKey = {
    courses: (options: GetCoursesOptions) =>
        `courses-${JSON.stringify(options)}`,
...
export async function createCourse(data: CourseFormValues) {
    try {
        ...
        if (data.selectedModules && data.selectedModules.length > 0) {
            const courseModulesData = data.selectedModules.map(
                (moduleId, index) => ({
                    id: nanoid(),
                    courseId: courseId,
                    moduleId: moduleId,
        ...
        if (data.selectedStudents && data.selectedStudents.length > 0) {
            const courseStudentsData = data.selectedStudents.map(
                (studentId) => ({
                    id: nanoid(),
                    courseId: courseId,
                    studentId: studentId,
            ...
            try {
                ...
                if (primarySkill && primarySkill.length > 0) {
                    whereConditions.push(
                        inArray(courses.primarySkill, primarySkill)
                    );
                }
        ...
        const cachedGetRecentAssignments = cache(
            async (limitParam: number, userIdParam?: string) => {
                ...
                const assignmentsResult = await db.query.assignments.findMany({
                    where: userIdParam
                        ? (assignments, { eq }) =>
                              eq(assignments.assignedBy, userIdParam)
                        : undefined,
                ...
                const courseAssignmentsData = await db
                    ...
                    .where(
                        inArray(courseAssignments.assignmentId, assignmentIds)
                    );

                // Membuat map dari assignmentId ke courseId
                const assignmentToCourseMap = new Map(
                    courseAssignmentsData.map((ca) => [
                        ca.assignmentId,
                        ca.courseId,
                    ])
                );
                ...
                const recentAssignments: RecentAssignment[] =
                    assignmentsResult.map((assignment) => {
                        const courseId = assignmentToCourseMap.get(
                            assignment.id
                        );
                        const courseTitle = courseId
                            ...
                            : "Unknown";
                        const totalStudents =
                            25 + Math.floor(Math.random() * 10); // Dummy data
