  ...
  const cachedGetAssignments = cache(
    async (opts: GetAssignmentsOptions) => {
      try {
        ...
        if (search) {
          whereConditions.push(ilike(assignments.title, `%${search}%`));
        }

        if (assignedBy) {
          whereConditions.push(eq(assignments.assignedBy, assignedBy));
        }

        if (isPublished !== undefined) {
          whereConditions.push(eq(assignments.isPublished, isPublished));
        }

        if (isChallenge !== undefined) {
          whereConditions.push(eq(assignments.isChallenge, isChallenge));
        }
        ...
        if (dueDateRange && dueDateRange.length > 0) {
          if (dueDateRange.length === 1) {
            ...
            whereConditions.push(sql`${assignments.dueDate} >= ${startOfDay}`);
            whereConditions.push(sql`${assignments.dueDate} <= ${endOfDay}`);
            ...
            whereConditions.push(sql`${assignments.dueDate} >= ${startDate}`);
            whereConditions.push(sql`${assignments.dueDate} <= ${endDate}`);
          }
        }
        ...
        return {
          ...
          totalCount: 0,
          error:
            error instanceof Error ? error.message : 'Gagal mengambil tugas',
        };
      }
    },
  ...
  try {
    ...
    if (userId && assignment.assignedBy !== userId) {
      return {
        assignment: null,
        error:
          'Anda tidak memiliki izin untuk mengubah status publikasi tugas ini',
      };
    }
