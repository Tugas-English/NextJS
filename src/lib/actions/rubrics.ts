...
const getCacheKey = {
    rubrics: (options: GetRubricsOptions) =>
        `rubrics-${JSON.stringify(options)}`,
...
export async function getRubricById(id: string) {
    const cachedGetRubric = cache(
        ...
        {
            tags: [
                CACHE_TAGS.RUBRIC_DETAIL,
                `${CACHE_TAGS.RUBRIC_DETAIL}-${id}`,
            ],
...
export async function toggleRubricDefaultStatus(id: string, userId: string) {
    try {
        ...
        if (rubric.createdBy !== userId) {
            return {
                rubric: null,
                error: "Anda tidak memiliki izin untuk mengubah status default rubrik ini",
            };
        }
...
export async function getUserDefaultRubric(userId: string) {
    const cachedGetDefaultRubric = cache(
        ...
        {
            tags: [
                CACHE_TAGS.USER_RUBRICS,
                `${CACHE_TAGS.USER_RUBRICS}-${userId}`,
            ],
            revalidate: 300, // 5 minutes
        }
    );
